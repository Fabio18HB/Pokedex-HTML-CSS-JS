...
async function loadPokemons() {
    ...
    for (let i = topo - 39; i <= topo; i++) {
        ...
        div.innerHTML += `<div class="col">
                    <a href="detalhes.html?numero=${pokemon.id} class="card" style="background-color:${getCor(pokemon.types[0].type.name)}">
                    ...
                    <h3 calss="card-title">${capitalizeFirstLetter(pokemon.name)}</h3>
...
window.onscroll = () => {
    if (
        window.innerHeight + window.scrollY >= document.body.scrollHeight - 500 && ! loading && !filtered
    ) {
        topo += 40; 
        loadPokemons();
    }
